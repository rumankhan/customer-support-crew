          ...
          <div className="mt-2 space-y-2" role="alert">
            ...
            {onEscalateCta ? (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-2 py-2">
                <p className="text-xs text-ink">{UI.escalateCtaHint}</p>
                <button
                  type="button"
                  className="mt-1 text-xs font-medium text-accent underline"
                  onClick={onEscalateCta}
                >
                  {UI.escalateCta}
                </button>
              </div>
...
export function ChatWindow({
  ...
  // Pending bubble only while running — never leave "Writing a reply" after COMPLETE.
  const pending =
          ...
          <>
            {turns.map((entry) => (
              <div key={entry.runId} className="space-y-2">
                ...
                <AgentBubble
                  result={entry.result}
                  onEscalateCta={entry.result.error != null ? onEscalateCta : undefined}
                />
              </div>
            ))}
            {pending ? (
